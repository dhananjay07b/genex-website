...
/** Maps every `ICON_CHOICES` value from backend/pages/blocks.py to its MUI icon component. */
export const MUI_ICON_REGISTRY: Record<string, MuiIconComponent> = {
...
/** Looks up a `ChoiceBlock(choices=ICON_CHOICES)` icon name, falling back to a default icon. */
export function getMuiIcon(name: string | null | undefined, fallback: MuiIconComponent = HelpOutlinedIcon): MuiIconComponent {
  if (!name) return fallback
  return MUI_ICON_REGISTRY[name] ?? fallback
}
